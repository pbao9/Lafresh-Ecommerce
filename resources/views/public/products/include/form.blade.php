<div class="detail-products">
    <div class="mb-1 fs-4">
        <h1 class="text-uppercase fw-bold title-products lh-base fs-1">
            {{ __($product->name . ' (' . $product->unit->description() . ') ') }}</h1>
        ...
        <div class="mb-3 d-flex align-items-center">
            <span class="fw-bold">@lang('Tình trạng'):</span><span
                @class(['badge ms-2', $product->badgeStock()])>{{ $product->labelStock() }}</span>
            {{-- {{ $product->in_stock }} --}}
        </div>
    </div>
            ...
            <div class="d-inline-flex gap-2 algin-items-center flex-wrap">
                <div class="d-flex align-items-center gap-2">
                    ...
                    <div class="input-group" style="width: 150px">
                        ...
                        <input type="number" id="input" name="quantity" value="0"
                ...
                @if (auth()->check() && auth()->user()->isAgent())
                    <div class=" gap-2 col-12 flex-column ">
                        <button id="level1" class="btn input-group-text">Số lượng chiết khấu cấp 1</button>
                        <button id="level2" class="btn input-group-text">Số lượng chiết khấu cấp 2</button>
                    </div>
                @endif
                @if (auth()->check() && auth()->user()->isSeller())
                    <div class="d-flex gap-2 col-12">
                        <button type="button" id="seller_donate" class="btn input-group-text">Tự động chọn số lượng
                            khuyến
                            mãi
                        </button>
                    </div>
                ...
                <div class="d-flex gap-2">
                    <button id="addtoCart" type="submit"
                        class="btn btn-outline-cyan fw-bold text-uppercase btn-add-products">
...
<script>
    function handleClick(event) {
        ...
        if (event.target.id === 'level1') {
            result = Math.ceil(10000000 / pricePromotion);
        } else if (event.target.id === 'level2') {
            result = Math.ceil(5000000 / pricePromotion);
        }
...
<script>
    function handleClick(event) {
        ...
        let qty = document.getElementById('input').value.trim();
...
<script>
    ...
    function updateBuyItUrl(qty) {
        var buyitUrl = "{{ route('checkout.index', ['product_id' => $product->id]) }}";
        buyitUrl += "&qty=" + qty;
        document.getElementById('buyit').setAttribute('href', buyitUrl);
    }
</script>
