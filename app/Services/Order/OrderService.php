...
class OrderService implements OrderServiceInterface
{
    ...
    public function payCart($request, $repoCart, $repoOrders, $repoProduct)
    {
        ...
        if ($roles === UserRoles::Agent) {
            ...
            foreach ($cartProducts as $cartProduct) {
                ...
                if ($cartProduct->price_selling >= 10000000) $lever = 1; 
                elseif ($cartProduct->price_selling >= 5000000) $lever = 2;

                $discount = $this->calculateDiscount($lever, $product->unit, $cartProduct->price_selling, $discountUser);
                $priceCart -= $discount;
    ...
    public function calculateDiscount($lever, $unitName, $price, $discountUser)
    {
        ...
        if ($lever === 1 || $lever === 2) {
            if($unitName->name === 'Pail') $discountPercent = $discountData['pail']/100;
            if($unitName->name === 'Bottle') $discountPercent = $discountData['bottle']/100;
