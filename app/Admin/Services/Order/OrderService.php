...
class OrderService implements OrderServiceInterface
{
    ...
    public function store(Request $request)
    {
        ...
        try {
            ...
            $discountPercentage = $user->discount_user ?? 0;
            if ($userLevel) {
            ...
            if (!empty($product)) {
                ...
                $subTotalPail += $this->totalPrice(
                    $this->repositoryProduct->getByIdsAndOrderByIds(
                        array_column($product, 'id'),
                        ['purchaseQty']
                    ),
    ...
    public function update(Request $request)
    {
        ...
        if ($status == 3) {
            $user = $this->repositoryUser->find($order->user_id);
            ...
            $point = [];
            $bonus = [];
            $BonusList = BonusPolicyDetail::where('bonus_policy_id', $unit)->get();
            foreach ($BonusList as $item) {
                $point[] = $item->point;
                $bonus[] = $item->bonus;
            }

            $bonusSale = BonusSale::where('user_id', $order->user_id)->first();
                ...
                if (!$bonusSale) {
                    ...
                    if ($unit == 1) {
                        ...
                        if ($qtyPail < 100) {
                            ...
                            for ($i = count($point) - 1; $i >= 0; $i--) {
                                if ($qtyPail >= $point[$i]) {
                                    $reward = $qtyPail * $bonus[$i];
                                    break;
                                } elseif ($qtyPail < $point[$i] && $qtyBottle >= $point[$i - 1]) {
                                    $reward = $qtyPail * $bonus[$i - 1];
                                    break;
                                }
                            }
                        ...
                        if ($qtyBottle < 500) {
                            ...
                            for ($i = count($point) - 1; $i >= 0; $i--) {
                                if ($qtyBottle >= $point[$i]) {
                                    $reward = $qtyBottle * $bonus[$i];
                                    break;
                                } elseif ($qtyBottle < $point[$i] && $qtyBottle >= $point[$i - 1]) {
                                    $reward = $qtyBottle * $bonus[$i - 1];
                                    break;
                                }
                            }
                        }
                    ...
                    $bonusData = [
                        ...
                        'month' => now(),
                        'qty_pail' => $qtyPail,
                        'qty_bottle' => $qtyBottle,
                        'reward' => $reward,
                    ];

                    $bonusSaleResult = BonusSale::create($bonusData);
                } else {
                    // day la update tren user_id neu da co bonusale r
                    $updatedQtyPail = $bonusSale->qty_pail;
                    $updatedQtyBottle = $bonusSale->qty_bottle;
                    $updatedReward = $bonusSale->reward;

                    if ($unit == 1) {
                        $updatedQtyPail += $totalQty;
                        if ($totalQty < 100) {
                            $updatedReward += 0;
                        } else {
                            for ($i = count($point) - 1; $i >= 0; $i--) {
                                if ($totalQty >= $point[$i]) {
                                    $updatedReward += $totalQty * $bonus[$i];
                                    break;
                                } elseif ($totalQty < $point[$i] && $totalQty >= $point[$i - 1]) {
                                    $updatedReward += $totalQty * $bonus[$i - 1];
                                    break;
                                }
                            }
                    ...
                    $bonusSale->update([
                        'qty_pail' => $updatedQtyPail,
                        'qty_bottle' => $updatedQtyBottle,
